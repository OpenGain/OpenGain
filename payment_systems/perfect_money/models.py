...
def validate_pm_wallet(wallet):
    if not re.match('U[0-9]{7}$', wallet):
        raise ValidationError(_('Кошелек должен иметь формат U1234567'))
...
class PSForUser(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, verbose_name=_('Пользователь'), null=False, blank=False,
                                related_name=NAME)
    wallet = models.CharField(_('Кошелек Perfect Money'), default=None, null=True, blank=True, max_length=8,
                              validators=[validate_pm_wallet], db_index=True)
